...
def verifica_erro(pesos,individuo,classe):
	...
	x1 = int(individuo[:1])
	x2 = int(individuo[2:3])
	x3 = int(individuo[4:])
		
	soma += x1*pesos[1] + x2*pesos[2] + x3*pesos[3] + pesos[0]
...
def corrige_pesos(pesos, taxa, erro, individuo):
	...
	x = [1, int(individuo[:1]), int(individuo[2:3]), int(individuo[4:])]
...
def main():
	...
	pesos = [0.1, 0.2, 0.3, 2.4]
	taxa = 0.8
	treinando = True
	
	header, individuos, classes = ler_arquivo('perceptron_3_var.csv')
	arq = open('pesos_perceptron_3_var.txt', 'w')
